...
test('typography and settings density follow the global UI rules', async () => {
  ...
  assert.match(components, /--ui-font-page:\s*1rem/);
  ...
  assert.match(components, /--page-pad:\s*var\(--ui-page-padding\)/);
  ...
  assert.match(components, /--ui-group-gap:\s*var\(--ui-space-5\)/);
