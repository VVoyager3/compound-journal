...
import { assessmentQuestions, scoreAssessment, scoreDimensionAssessment } from '../src/assessment.ts';
...
test('one dimension can be scored without answering the other four', () => {
  const questions = assessmentQuestions(30);
  const energyQuestions = questions.filter((question) => question.dimension === 'energy');
  const answers = Object.fromEntries(energyQuestions.map((question) => [question.id, question.reverse ? 1 : 5]));
  assert.equal(scoreDimensionAssessment('energy', questions, answers), 100);
  assert.throws(() => scoreDimensionAssessment('mind', questions, answers), /回答全部问题/);
});
