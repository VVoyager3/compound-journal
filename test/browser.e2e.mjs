...
test('all top-level page titles align to the same left edge as Today', async () => {
  ...
  try {
    ...
    for (const [route, pageClass] of [
      ['today', 'page-today'], ['calendar', 'page-calendar'], ['record', 'page-record'], ['growth', 'page-growth'],
      ['system', 'page-system'], ['tasks', 'page-tasks'], ['review', 'page-review'], ['day/2026-08-24', 'page-day'],
    ]) {
      await page.goto(`${baseUrl}/#/${route}`);
      await page.locator(`.${pageClass} > .page-header h1`).waitFor();
...
test('expanded settings avoid permanent explanatory paragraphs', async () => {
  ...
  try {
    ...
    await page.getByText('每次周复盘默认包含的信息', { exact: true }).click();
    assert.equal(await page.getByText('选择周复盘可以使用的数据。日记原文不在其中。', { exact: true }).count(), 0);
    await page.getByText('分类与行动规则', { exact: true }).click();
    assert.equal(await page.getByText('还没有经过确认的结论。', { exact: true }).count(), 0);
    assert.equal(await page.getByText('确认前不会参与建议。', { exact: true }).count(), 0);
    assert.equal(await page.getByText('暂时没有待确认内容。', { exact: true }).count(), 0);
    await page.getByText('待你核对 · 0', { exact: true }).click();
...
test('today keeps records and habit editing behind compact entry points', async () => {
  ...
  try {
    ...
    await assert.doesNotReject(() => page.getByRole('button', { name: '暂停“晚饭后散步”的计划日打卡' }).waitFor());
    await page.goto(`${baseUrl}/#/today`);
    ...
    await editDialog.getByText('更多设置（可选）', { exact: true }).click();
    await editDialog.getByRole('textbox', { name: '习惯名称' }).fill('晚饭后散步十五分钟');
    await editDialog.getByRole('textbox', { name: '最小动作' }).fill('先走三分钟');
    await editDialog.getByRole('combobox', { name: '习惯状态' }).selectOption('paused');
    await editDialog.getByRole('button', { name: '保存习惯' }).click();
    await page.goto(`${baseUrl}/#/today`);
    await assert.doesNotReject(() => page.getByText('晚饭后散步十五分钟', { exact: true }).waitFor());
    await assert.doesNotReject(() => page.getByText('已暂停', { exact: true }).waitFor());
    assert.equal(await page.getByText('晚饭后散步', { exact: true }).count(), 0, 'today must not keep the old habit snapshot');
    await page.goto(`${baseUrl}/#/tasks`);
    await openTaskView(page, '计划');
    await page.getByRole('button', { name: '开始“晚饭后散步十五分钟”的计划日打卡' }).click();
    await page.goto(`${baseUrl}/#/today`);
    await assert.doesNotReject(() => page.getByRole('button', { name: '完成打卡：晚饭后散步十五分钟' }).waitFor());
    assert.deepEqual(apiRequests, []);
  } finally {
    await context.close();
  }
...
test('the bottom quick-add keeps task creation in context', async () => {
  ...
  try {
    ...
    const quick = page.getByRole('textbox', { name: '添加今日任务' });
    await quick.focus();
    await page.locator('#task-view-today.is-adding-task').waitFor();
    const existingRow = page.locator('.task-list-item').filter({ hasText: '已有任务' });
    await existingRow.evaluate((element) => Promise.all(element.getAnimations().map((animation) => animation.finished)));
    const [rowOpacity, formOpacity] = await Promise.all([
      existingRow.evaluate((element) => Number(getComputedStyle(element).opacity)),
      page.locator('.task-quick-add').evaluate((element) => Number(getComputedStyle(element).opacity)),
    ]);
    assert.ok(rowOpacity < .5, 'existing tasks should recede while adding');
    assert.equal(formOpacity, 1, 'the add row should stay prominent');
    const quickAddLayout = await page.locator('.task-quick-add').evaluate((element) => {
      const form = element.getBoundingClientRect();
      const navigation = document.querySelector('.bottom-nav')?.getBoundingClientRect();
      const submit = element.querySelector('.task-quick-submit');
      const submitBox = submit?.getBoundingClientRect();
      const plus = submit ? getComputedStyle(submit, '::before') : null;
      return {
        position: getComputedStyle(element).position,
        gapFromNavigation: navigation ? navigation.top - form.bottom : null,
        submitSize: submitBox ? [submitBox.width, submitBox.height] : null,
        plusSize: plus ? [plus.width, plus.height] : null,
      };
    });
    assert.equal(quickAddLayout.position, 'fixed', 'quick add should stay in the bottom row above navigation');
    assert.ok(quickAddLayout.gapFromNavigation !== null && Math.abs(quickAddLayout.gapFromNavigation) <= 1, 'quick add should sit directly above navigation');
    assert.deepEqual(quickAddLayout.submitSize, [44, 44], 'the add action should retain a safe touch target');
    assert.deepEqual(quickAddLayout.plusSize, ['32px', '32px'], 'the visible plus should stay visually compact');
    await quick.fill('直接添加');
    await page.getByRole('button', { name: '安排任务', exact: true }).click();
    ...
    await page.getByText(/已反馈 ·/).click();
...
test('task cards and settings sections do not retain a thick blue focus frame', async () => {
  ...
  try {
    ...
    await page.goto(`${baseUrl}/#/system`);
    const settingsSummary = page.locator('.settings-disclosure').filter({ hasText: '状态自评' }).locator(':scope > summary');
    ...
    }, 'settings disclosures should replace the blue focus shadow and touch highlight with a light underline');
  } finally {
    await context.close();
  }
});
...
test('task, goal, and habit deletion lives on each specific item', async () => {
  ...
  try {
    ...
    await goalDialog.getByRole('textbox', { name: '你想让什么事情发生？' }).fill('完成数学错题复习计划');
    ...
    await goalDialog.getByRole('button', { name: '只保存计划' }).click();
    ...
    await item.getByText('更多', { exact: true }).click();
...
test('secondary pages stay concise before the user has evidence', async () => {
  ...
  try {
    ...
    await page.goto(`${baseUrl}/#/calendar`);
    await assert.doesNotReject(() => page.getByText('记录不足', { exact: true }).waitFor());
...
test('success diary prompts stay optional and AI goal decomposition requires confirmation', async () => {
  ...
  try {
    ...
    await assert.doesNotReject(() => page.getByText('目标已建立，下一步已放入今天。', { exact: true }).waitFor());
    ...
    await assert.doesNotReject(() => page.locator('.goal-row').filter({ hasText: '发布一篇文章' }).getByText('更多', { exact: true }).waitFor());
    ...
    await assert.doesNotReject(() => page.getByText('1 项待完成', { exact: true }).waitFor());
    ...
    await openTaskView(page, '计划');
    await page.locator('.goal-row').filter({ hasText: '发布一篇文章' }).getByText('更多', { exact: true }).click();
    ...
    await assert.doesNotReject(() => page.getByRole('dialog', { name: '收束今天' }).getByText(/明日一步|明日待定/).waitFor());
  } finally {
    await context.close();
  }
});
...
test('manual milestone badges are evidence-first, reversible, and survive goal status changes', async () => {
  ...
  try {
    ...
    await goalCard.getByText('更多', { exact: true }).click();
    ...
    for (const [index, [name]] of milestones.entries()) {
      ...
      assert.equal(await goalCard.getByText('已完成 · +50 经验', { exact: true }).count(), index + 1, 'one tap must complete only one milestone');
...
test('system-retired BONUS and capacity candidates stay read-only until the same milestone action is scheduled', async () => {
  ...
  try {
    ...
    await blockingFeedback.getByRole('combobox', { name: '结果' }).selectOption('skipped');
    ...
    await assert.doesNotReject(() => page.getByText('待安排', { exact: true }).waitFor());
...
test('the companion explains the current direction before offering adjustments', async () => {
  ...
  try {
    ...
    assert.equal(await companionButton.getAttribute('aria-expanded'), 'false');
    assert.equal(await page.locator('.main-action').getByText('这是主线的可追溯理由。', { exact: true }).count(), 0);
    await assert.doesNotReject(() => page.locator('.main-action').getByRole('heading', { name: '验证主线依据' }).waitFor());
    await page.goto(`${baseUrl}/#/system`);
    await page.getByText('分类与行动规则', { exact: true }).click();
...
test('low state proposes replaceable recovery and one-click no-penalty feedback', async () => {
  ...
  try {
    ...
    await skipDialog.getByRole('combobox', { name: '结果' }).selectOption('skipped');
...
test('completed action is traceable from its growth branch at 320px', async () => {
  ...
  try {
    ...
    await assert.doesNotReject(() => page.getByRole('heading', { name: '今天已有记录' }).waitFor());
...
test('room furniture walks end at the matching interaction anchors', async () => {
  const { context, page, apiRequests } = await freshPage();
  const destinations = [
    ['打开记录', 'desk', /#\/record$/, [0.31, 0.44, 0.68, 0.84], '--face-left-frame'],
...
test('Android users can request the desktop companion from settings once', async () => {
  ...
  try {
    ...
    await page.getByText('今日任务小组件', { exact: true }).click();
    await assert.doesNotReject(() => page.getByText('已添加', { exact: true }).waitFor());
...
test('task feedback lets the user review an AI candidate before XP is settled', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(await compactRow.innerText(), /只完成一个五分钟版本|5 分钟/, 'planning metadata belongs in details, not the main task row');
    await page.getByRole('button', { name: '查看任务：反馈闭环行动' }).click();
    const dialog = page.getByRole('dialog', { name: '记录任务结果' });
    await assert.doesNotReject(() => dialog.getByText('只完成一个五分钟版本', { exact: true }).waitFor());
    const dialogBox = await dialog.boundingBox();
    assert.ok(dialogBox && Math.abs(dialogBox.y + dialogBox.height / 2 - 400) <= 2, 'task details should open centered on mobile');
    ...
    await page.getByRole('button', { name: '查看任务：反馈闭环行动' }).click();
    await assert.doesNotReject(() => dialog.getByRole('textbox', { name: '做了什么（可选）' }).waitFor());
    assert.equal(await dialog.getByRole('textbox', { name: '做了什么（可选）' }).inputValue(), '完成了一部分可核对步骤。');
    assert.deepEqual(apiRequests, []);
...
test('weekly review sends summaries instead of journals and adopts its candidate in one click', async () => {
  ...
  try {
    ...
    await assert.doesNotReject(() => page.getByText('下周小尝试', { exact: true }).waitFor());
...
test('today, companion, day review, and task board prefer the real pending MAIN', async () => {
  ...
  try {
    ...
    await partialDialog.getByRole('combobox', { name: '结果' }).selectOption('partial');
...
test('a newly created BONUS has no historic debt and remains usable in weekly review', async () => {
  ...
  try {
    ...
    assert.match(momentum ?? '', /最近坚持程度 5\/5/);
