import { actionButton, actionGroup, formStack, infoRow, labelledControl, listGroup, listRow, listSection, segmentedControl, segmentedItem, taskRow } from '../src/ui-list.ts';
...
function render() {
  ...
  const tabs = segmentedControl('nav'); tabs.setAttribute('aria-label', '样例分段');
  for (const text of ['日历','本周','成长']) {
    const tab = segmentedItem('button', text, { active: text === '本周' });
  ...
  for (const [tone, text] of [['primary','保存'],['secondary','取消'],['quiet','修改'],['danger','删除']] as const) {
    actions.append(actionButton(text, () => report('样例操作，不修改真实数据'), { variant: tone }));
  }
  common.append(listSection('按钮', {}, actions));
  const form = formStack();
  for (const [text, tag] of [['名称','input'],['日期','input'],['备注','textarea']]) {
    ...
    form.append(labelledControl(text, control));
  }
