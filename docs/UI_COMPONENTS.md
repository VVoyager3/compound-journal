...
复盘输入最低116px，重复标题到星期16px，热力图16px方格/5px间隔，分析列表默认100%内容宽度。热力图统一使用暖灰底格与同一深绿浓度阶梯：任务分析仅以浓度表达每日数量；单习惯分析以小勾表示完成、描边短横表示漏打、底格表示未计划；不显示颜色图例。今天使用与格子留有间隙的独立外框，不覆盖原状态。400px仅为审查视口。
