...
复盘输入最低116px，重复标题到星期16px，热力图13px方格/3px间隔，分析列表默认100%内容宽度。热力图以紧凑的横向时间纹理呈现：暖灰低对比底格退后，同一深绿浓度阶梯突出完成数据，格内不放勾、短横或其他符号；单习惯漏打只使用轻描边区分。页面不显示颜色图例，区块底部不使用分割线。今天使用细内框，不改变方格尺寸。400px仅为审查视口。
